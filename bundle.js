...
var gracefulFs = createCommonjsModule(function (module) {
  ...
  if (!commonjsGlobal[gracefulQueue]) {
    ...
    fs__default.close = function (fs$close) {
    ...
    fs__default.closeSync = function (fs$closeSync) {
      function closeSync(fd) {
        // This function uses the graceful-fs shared queue
        fs$closeSync.apply(fs__default, arguments);
        retry();
      }
...
var lib = createCommonjsModule(function (module) {
  ...
  if (Object.getOwnPropertyDescriptor(fs__default, 'promises')) {
    Object.defineProperty(module.exports, 'promises', {
      get() {
        return fs__default.promises;
      }
...
var configJson = lib.readJsonSync('./i18nAid.config.json', {
  "throws": false
});
...
var readdirSync = lib.readdirSync,
    ensureDirSync = lib.ensureDirSync;
/**
 * 获取语言资源的根目录
 */
  ...
  function traverseInner(obj, cb, path) {
    lodash.forEach(obj, function (val, key) {
      if (typeof val === 'string') {
        cb(val, [].concat(_toConsumableArray(path), [key]).join('.'));
      } else if (_typeof(val) === 'object' && val !== null) {
        traverseInner(val, cb, [].concat(_toConsumableArray(path), [key]));
      }
    });
  }
...
function getAllMessages() {
  ...
  var srcLangDir = path.resolve(getLangsDir(), lang);
  ensureDirSync(srcLangDir);
  var files = readdirSync(srcLangDir);
  files = files.filter(function (file) {
    return file.endsWith(config.fileType) && file !== "index".concat(config.fileType);
  }).map(function (file) {
    return path.resolve(srcLangDir, file);
  });
  var allMessages = files.map(function (file) {
    var _require = require(file),
        messages = _require["default"];

    var fileNameWithoutExt = path.basename(file).split('.')[0];
    var flattenedMessages = {};
    traverse(messages, function (message, path) {
      var key = fileNameWithoutExt + '.' + path;
  ...
  return Object.assign.apply(Object, [{}].concat(_toConsumableArray(allMessages)));
}
...
function initMessages(lang) {
  ...
  var targetMessages = getAllMessages(lang);

  var effectiveMessages = lodash.pickBy(targetMessages, function (message, key) {
    return allMessages.hasOwnProperty(key);
  });

  var extraMessages = lodash.pickBy(allMessages, function (message, key) {
    return !effectiveMessages.hasOwnProperty(key);
  });

  var messagesToImport = Object.assign({}, effectiveMessages, extraMessages);

  var keysByFiles = lodash.groupBy(Object.keys(messagesToImport), function (key) {
    return key.split('.')[0];
  });

  var messagesByFiles = lodash.mapValues(keysByFiles, function (keys, file) {
    var rst = {};

    lodash.forEach(keys, function (key) {
      lodash.setWith(rst, key.substr(file.length + 1), messagesToImport[key], Object);
    });

    return rst;
  });

  lodash.forEach(messagesByFiles, function (messages, file) {
    writeMessagesToFile(messages, file, lang);
  });
}

function writeMessagesToFile(messages, file, lang) {
  var srcMessages = require(path.resolve(getLangsDir(), 'zh_CN', file))["default"];

  var dstFile = path.resolve(getLangsDir(), lang, file);
  var rst = {};
  traverse(srcMessages, function (message, key) {
    lodash.setWith(rst, key, lodash.get(messages, key), Object);
  });
  fs.writeFileSync(dstFile + config.fileType, 'export default ' + JSON.stringify(rst, null, 2));
}
...
function exportMessages(file, lang) {
  var allMessages = getAllMessages();
  var existingTranslations = getAllMessages(lang, function (message, key) {
    return allMessages[key] !== message;
  });
  var messagesToTranslate = Object.keys(allMessages).filter(function (key) {
    return !existingTranslations.hasOwnProperty(key);
  }).map(function (key) {
  ...
  var content = tsvFormatRows(messagesToTranslate);
  fs.writeFileSync(file, content);
  console.log("\u5BFC\u51FA ".concat(messagesToTranslate.length, " \u6761\u672A\u7FFB\u8BD1\u5B57\u6BB5."));
}

function getMessagesToImport(file) {
  var content = fs.readFileSync(file).toString();
  var messages = tsvParseRows(content, function (_ref) {
    ...
    try {
      ...
      throw new Error("Illegal message: ".concat(value));
    }
  ...
  if (duplicateKeys.size > 0) {
    var errorMessage = 'Duplicate messages detected: \n' + _toConsumableArray(duplicateKeys).join('\n');
...
function importMessages(file, lang) {
  var messagesToImport = getMessagesToImport(file);
  var allMessages = getAllMessages();
  messagesToImport = lodash.pickBy(messagesToImport, function (message, key) {
    return allMessages.hasOwnProperty(key);
  });
...
function writeMessagesToFile$1(messages, file, lang) {
  ...
  var oldDstMessages = require(dstFile)["default"];

  var rst = {};
  traverse(srcMessages, function (message, key) {
    lodash.setWith(rst, key, lodash.get(messages, key) || lodash.get(oldDstMessages, key), Object);
  });
  fs.writeFileSync(dstFile + config.fileType, 'export default ' + JSON.stringify(rst, null, 2));
}
